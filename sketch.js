...
function addNewBackgroundTiles(){
  if (backgroundTiles.length < 3){
    ...
    let bgLoop = createSprite(currentBackgroundTilePosition, height/2, width,height)
...
function removeOldBackgroundTiles(){
  ...
  function addNewPlatforms(){
    if (platformsGroup.length < 5){
      ...
      let platform = createSprite((currentPlatformLocation * 1.2), random(300,400), 1132, 336)
  ...
  function jumpDetection(){
    if(keyWentDown(UP_ARROW) && (runner.velocity.y == 0 || runner.velocity.y == 1)){
